...
/* @author: Jason Harris modified to add functionality to remove cached bitmaps by key name with remove(key) and remove(key, file) */
...
public class DiskLruCache {
    ...
    /**
     * Removes disk cache entry for the given key. 
     *
     * @param key A unique identifier for the bitmap.
     */
    public void remove(String key) {
        if(containsKey(key)) {
            final String file = createFilePath(key);
            synchronized (mLinkedHashMap) {
    ...
    private void remove(String key, String file) {
    ...
    public Bitmap get(String key) {
        synchronized (mLinkedHashMap) {
            ...
            if (file != null) {
                if (BuildConfig.DEBUG) {
                    Log.d(TAG, "Disk cache hit - file: " + file);
                }
