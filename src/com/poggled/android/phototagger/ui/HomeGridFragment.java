 ...
 */
public class HomeGridFragment extends Fragment implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final File dir = new File(
				Utils.getExternalCacheDir(HomeGridFragment.this.getActivity()),
				ImageProcessingService.OUTPUT_DIR_NAME);
	...
	public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
		...
		// i.putExtra(PhotoDetailActivity.EXTRA_IMAGE, (int) id);
		i.putExtra("extra_image", (int) id);
		startActivity(i);
	}
		...
		case R.id.delete_all:
			...
			builder.setMessage(
					getResources().getQuantityString(
							R.plurals.dialog_delete_all_text, count, count))
								...
								public void onClick(DialogInterface dialog,
									...
									ProgressDialog progress = ProgressDialog
											.show(getActivity(),
													...
													getString(R.string.dialog_delete_all_progress_text),
													true, true);
									deleteAll();
									progress.dismiss();
								}
	...
	private void deleteAll() {
		...
		File eyeFiDir = new File(Environment.getExternalStorageDirectory()
				+ File.separator + "Eye-Fi");
		for (File file : files) {
			...
			if (file.exists() && BitmapUtils.isImage(file)) {
				if (BuildConfig.DEBUG) {
					Log.i(getClass().getName(),
							"PhotoGrid: Received notification file written, deleted, or moved: "
									+ file.getName());
				}
				// update the ui; ui should always be updated from the main
				// thread
				mHandler.post(new Runnable() {
					public void run() {
						if (event == FileObserver.MOVED_FROM
								| event == FileObserver.DELETE) {
							mImageWorker.getImageCache().removeBitmapFromCache(
